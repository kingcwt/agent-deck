---
name: kc-slim-review
display_name: Slim Review
description: Review code, diffs, or a proposed implementation for over-engineering and unnecessary scope. Use when the user types /kc-sr, /kc-slim-review, $kc-sr, $kc-slim-review, kc-sr, asks whether code can be simpler, asks for a minimal refactor review, or wants an AI agent to find unnecessary abstractions, dependencies, duplicate logic, shared-code overreach, speculative configuration, or opportunistic cleanup. Default to read-only review; only edit code when the user explicitly asks to apply fixes after the review.
short_description: Find smaller safe implementations [最小实现审查]
default_prompt: Use $kc-sr to review code or a diff for over-engineering, duplicated logic, avoidable dependencies, and unsafe scope expansion without changing code by default.
codex_names: kc-sr,kc-slim-review
claude_skill_names: kc-sr,kc-slim-review
claude_commands: kc-sr,kc-slim-review
